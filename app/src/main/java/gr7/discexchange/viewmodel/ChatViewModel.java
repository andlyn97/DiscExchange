...
public class ChatViewModel extends ViewModel {
    ...
    public void addMessage(Message message) {
        ...
        ref.limit(1).get().addOnCompleteListener(task -> {
            if(!task.isSuccessful()) {
                return;
            }
        ...
        ref.add(message).addOnCompleteListener(task -> {
            if(task.isSuccessful()) {
                return;
            }
            DocumentReference docRef = task.getResult();
            Message message1 = docRef.get().getResult().toObject(Message.class);

            List<Message> tempMessagesList = messages.getValue();
            tempMessagesList.add(message1);
